...
class MAPPO:
    ...
    def _choose_sandra_action_with_filter(
        ...
        for candidate in candidates:
            ...
            score = worst_risk + 0.3 * predicted_risk + self.sandra_rewrite_penalty * rewrite
            if hard_unsafe:
                score += 1000.0
            item = {
        ...
        safe_candidates = [x for x in evaluations if not x["hard_unsafe"]]
    ...
    def _maybe_apply_sandra_overrides(self, rl_actions, action_mask, risk_vector):
        ...
        for agent_id in range(min(len(final_actions), len(risk_vector))):
            ...
            if self.sandra_log_decisions and filter_detail is not None:
                print(
                    "[SANDRA FILTER] car_no=%d risk_before=%.4f selected=%d predicted_risk=%.4f worst_risk=%.4f rewrite=%s crash=%s score=%.4f source=%s"
                    % (
                        agent_id + 1,
                        risk_value,
            ...
            if self.sandra_log_decisions and self.sandra_log_candidates and filter_detail is not None:
                for cand in filter_detail.get("all_candidates", []):
                    print(
                        "[SANDRA CAND] car_no=%d cand=%d exec=%d pred=%.4f worst=%.4f rewrite=%s crash=%s score=%.4f done=%s hz=%d"
                        % (
                            agent_id + 1,
                            int(cand["candidate"]),
                            int(cand["executed"]),
