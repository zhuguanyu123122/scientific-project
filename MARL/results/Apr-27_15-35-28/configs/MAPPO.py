...
class MAPPO:
    ...
    def __init__(self, env, state_dim, action_dim,
        ...
        self.sandra_enabled = bool(sandra_enabled and self.llm_enabled)
    ...
    def interact(self):
        ...
        for i in range(self.roll_out_n_steps):
            ...
            if self.perception_enabled:
                ...
                risk_before_action = np.zeros((self.n_agents,), dtype=np.float32)
            ...
            if self.perception_enabled:
                ...
                step_risk = np.zeros((self.n_agents,), dtype=np.float32)
    ...
    def _maybe_apply_sandra_overrides(self, rl_actions, action_mask, risk_vector):
        if not self.sandra_enabled or not self.llm_enabled:
            return rl_actions
        env_obj = getattr(self.env, "unwrapped", self.env)
        ...
        for agent_id in range(min(len(final_actions), len(risk_vector))):
            ...
            decision = self.sandra_decider.decide_action(
                env=env_obj,
                car_index=agent_id,
            ...
            if isinstance(decision, tuple) and len(decision) == 3:
                llm_action, llm_source, llm_meta = decision
            elif isinstance(decision, tuple) and len(decision) == 2:
                llm_action, llm_source = decision
                llm_meta = {
                    "backend": "unknown",
                    "ranked_actions": [int(llm_action)],
                    "selected_action": int(llm_action),
                    "verification_results": [],
                }
            else:
                llm_action = int(final_actions[agent_id])
                llm_source = "fallback"
                llm_meta = {
                    "backend": "unknown",
                    "ranked_actions": [int(llm_action)],
            ...
            if self.sandra_log_decisions:
                print(
                    "[SANDRA 排序] 车辆=%d 后端=%s 排序动作=%s 已选=%d 来源=%s"
                    % (
                        agent_id + 1,
                ...
                for item in formal_results:
                    if not isinstance(item, dict):
                        continue
                    print(
                        "[SANDRA 验证] 车辆=%d 排名=%d 动作=%d 安全=%s 分数=%.4f 违规=%s"
                        % (
                            agent_id + 1,
                            int(item.get("rank", -1)),
            ...
            if int(sandra_action) != int(final_actions[agent_id]):
                ...
                if self.sandra_log_decisions:
                    print(
                        "[SANDRA 切换] 回合=%d 步=%d 车辆=%d 风险=%.4f RL动作=%d SanDRA动作=%d 来源=%s"
                        % (
                            self.n_episodes + 1,
            ...
            if self.sandra_log_decisions and filter_detail is not None:
                print(
                    "[SANDRA 筛选] 车辆=%d 风险前=%.4f 选中=%d 预测风险=%.4f 最坏风险=%.4f 被改写=%s 预测碰撞=%s 形式不安全=%s 分数=%.4f 来源=%s"
                    % (
                        agent_id + 1,
                        risk_value,
            ...
            if self.sandra_log_decisions and self.sandra_log_candidates and filter_detail is not None:
                for cand in filter_detail.get("all_candidates", []):
                    print(
                        "[SANDRA 候选] 车辆=%d 候选=%d 执行=%d 预测风险=%.4f 最坏风险=%.4f 被改写=%s 预测碰撞=%s 形式不安全=%s 分数=%.4f 结束原因=%s 预演步数=%d 违规=%s"
                        % (
                            agent_id + 1,
                            int(cand["candidate"]),
                            int(cand["executed"]),
        ...
        for idx in risk_indices.tolist():
            ...
            if vehicle_slot < len(self.env.controlled_vehicles):
                ...
                try:
                    ...
                    pass
            print(
                "[风险预警] 回合=%d 步=%d 车辆=%d 车辆ID=%d 风险=%.4f 阈值=%.4f 车道=%s 位置=(%.2f, %.2f)"
                % (
                    current_episode,
                    current_step,
                    vehicle_no,
