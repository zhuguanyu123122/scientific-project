...
class MAPPO:
    ...
    def __init__(self, env, state_dim, action_dim,
                 ...
                 risk_reward_enabled=False, risk_reward_lambda=0.0,
                 risk_reward_metric="post", risk_reward_scope="agent",
        ...
        self.risk_reward_enabled = bool(risk_reward_enabled)
        self.risk_reward_lambda = float(risk_reward_lambda)
        self.risk_reward_metric = str(risk_reward_metric)
        self.risk_reward_scope = str(risk_reward_scope)
        ...
        self.episode_mean_risk_penalty = []
    ...
    def interact(self):
        ...
        for i in range(self.roll_out_n_steps):
            ...
            reward, step_penalty = self._apply_risk_reward_shaping(reward, info, self.n_agents)
            rewards.append(reward)
            episode_risk_penalty_sum += step_penalty
            average_speed += info["average_speed"]
            step_risk = self._extract_agent_risk_vector(info, "agent_risk_post", self.n_agents)
        ...
        if done:
            ...
            if self.current_episode_risk_steps > 0:
                ...
                self.episode_mean_risk_penalty.append(episode_risk_penalty_sum / self.current_episode_risk_steps)
            else:
                self.episode_mean_risk.append(0.0)
                self.episode_agent_risk.append([0.0] * self.n_agents)
                self.episode_mean_risk_penalty.append(0.0)
            self.current_episode_risk_sum = 0.0
            self.current_episode_risk_steps = 0
            self.current_episode_agent_risk_sum = np.zeros((self.n_agents,), dtype=np.float32)
        else:
    ...
    def _apply_risk_reward_shaping(self, base_reward, info, n_agents):
        if n_agents <= 0:
            return np.zeros((0,), dtype=np.float32), 0.0
        reward_vec = np.asarray(base_reward, dtype=np.float32).reshape(-1)
        if reward_vec.shape[0] < n_agents:
            reward_vec = np.pad(reward_vec, (0, n_agents - reward_vec.shape[0]), mode="constant")
        elif reward_vec.shape[0] > n_agents:
            reward_vec = reward_vec[:n_agents]

        if not self.risk_reward_enabled or self.risk_reward_lambda <= 0:
            return reward_vec, 0.0

        risk_post = self._extract_agent_risk_vector(info, "agent_risk_post", n_agents)
        risk_delta = self._extract_agent_risk_vector(info, "agent_risk_delta", n_agents)
        if self.risk_reward_metric == "delta_pos":
            penalty_source = np.maximum(risk_delta, 0.0)
        else:
            penalty_source = risk_post

        if self.risk_reward_scope == "fleet_mean":
            penalty_vec = np.full((n_agents,), float(np.mean(penalty_source)), dtype=np.float32)
        else:
            penalty_vec = penalty_source

        penalty_value = self.risk_reward_lambda * penalty_vec
        return reward_vec - penalty_value, float(np.mean(penalty_value))
