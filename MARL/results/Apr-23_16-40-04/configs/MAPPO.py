...
class MAPPO:
    ...
    def _preview_joint_action(self, joint_actions, base_np_state=None, base_py_state=None):
        env_obj = getattr(self.env, "unwrapped", self.env)
        ...
        try:
            ...
            _, _, _, info = env_shadow.step(tuple(int(a) for a in joint_actions))
            executed = self._extract_executed_action(info, joint_actions, len(joint_actions))
            post_risk = self._extract_agent_risk_vector(info, "agent_risk_post", len(joint_actions))
            return {
                "executed": executed,
                "post_risk": post_risk,
                "info": info,
    ...
    def _choose_sandra_action_with_filter(
        ...
        candidates = [int(llm_action)] + [int(a) for a in available_actions if int(a) != int(llm_action)]
        best = None
        for candidate in candidates:
            ...
            score = predicted_risk + self.sandra_rewrite_penalty * rewrite
            item = {
        ...
        if (risk_before - best["predicted_risk"]) < self.sandra_min_risk_improve:
            # Not enough improvement; still return best executable action for consistency.
            return int(best["executed"]), "safety_filter_no_improve", best

        return int(best["executed"]), "safety_filter", best
    ...
    def _maybe_apply_sandra_overrides(self, rl_actions, action_mask, risk_vector):
        ...
        for agent_id in range(min(len(final_actions), len(risk_vector))):
            ...
            llm_action, llm_source = self.sandra_decider.decide_action(
                env=env_obj,
                car_index=agent_id,
            ...
            if self.sandra_log_decisions and filter_detail is not None:
                print(
                    "[SANDRA FILTER] car_no=%d risk_before=%.4f selected=%d predicted_risk=%.4f rewrite=%s score=%.4f"
                    % (
                        agent_id + 1,
                        risk_value,
