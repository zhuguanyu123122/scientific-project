...
def train(args):
    ...
    risk_reward_enabled = config.getboolean('TRAIN_CONFIG', 'risk_reward_enabled') \
        if config.has_option('TRAIN_CONFIG', 'risk_reward_enabled') else False
    risk_reward_lambda = config.getfloat('TRAIN_CONFIG', 'risk_reward_lambda') \
        if config.has_option('TRAIN_CONFIG', 'risk_reward_lambda') else 0.0
    risk_reward_metric = config.get('TRAIN_CONFIG', 'risk_reward_metric') \
        if config.has_option('TRAIN_CONFIG', 'risk_reward_metric') else "post"
    risk_reward_scope = config.get('TRAIN_CONFIG', 'risk_reward_scope') \
        if config.has_option('TRAIN_CONFIG', 'risk_reward_scope') else "agent"
    ...
    mappo = MAPPO(env=env, memory_capacity=MEMORY_CAPACITY,
                  ...
                  risk_reward_enabled=risk_reward_enabled,
                  risk_reward_lambda=risk_reward_lambda,
                  risk_reward_metric=risk_reward_metric,
                  risk_reward_scope=risk_reward_scope,
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            ...
            if len(mappo.episode_mean_risk_penalty) > 0:
                print(
                    "Episode %d, Recent Risk Penalty %.4f"
                    % (mappo.n_episodes + 1, float(np.mean(mappo.episode_mean_risk_penalty[-10:]))),
                    flush=True,
                )
            ...
            np.save(output_dir + '/{}'.format('average_risk_penalty'), np.array(mappo.episode_mean_risk_penalty))
